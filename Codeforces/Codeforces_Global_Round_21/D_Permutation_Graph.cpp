 ...
 *    created: 25.06.2022 21:41:10 IST
**/
#include <bits/stdc++.h>
using namespace std;
...
int inf = 1e9;
pair <int,int>  omx(pair <int,int>  a,pair <int,int>  b)
{
	return max(a,b);
}
...
pair <int,int>  omn(pair <int,int>  a,pair <int,int>  b)
{
	return min(a,b);
}
...
void solve()
{
	...
	segtree<pair <int,int> , omx,emx > mx(v);
	segtree<pair <int,int> , omn,emn > mn(v);

	function<int(int,int)> dp = [&](int l,int r)->int{
		if(l>=r)return 0;
		int Mn = mn.prod(l,r+1).second;
		int Mx = mx.prod(l,r+1).second;
		if(Mn>Mx)swap(Mn,Mx);
		return dp(l,Mn)+1+dp(Mx,r);
	};
