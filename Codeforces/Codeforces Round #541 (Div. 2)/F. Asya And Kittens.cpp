/*
	group : Codeforces - Codeforces Round #541 (Div. 2)
	name : F. Asya And Kittens.cpp
	srcPath : /home/aryan/Desktop/cp-workspace/F_Asya_And_Kittens.cpp
	url : https://codeforces.com/problemset/problem/1131/F
*/
/**
 *    author:  Aryan Agarwal
 *    created: 19.10.2021 19:30:55 IST
**/
#include <bits/stdc++.h>
using namespace std;
...
void solve()
{
	...
	for(int i=1;i<n;i++){
		...
		int lu=d.leader(u);
		int lv=d.leader(v);
		int newpar = d.merge(u,v);
		lastpar=newpar;
		adj[newpar].insert(adj[newpar].end(),adj[newpar^lu^lv].begin(),adj[newpar^lu^lv].end());
	}

	for(int x : adj[lastpar]){
		cout<<x+1<<" ";
	}
