 ...
 *    created: 15.11.2021 11:21:42 IST
**/
...
pair <mint,mint> val[mxn+1][mxbit+1];
int tin[mxn+1],tout[mxn+1],height[mxn+1];
int T;
...
int lca(int u,int v){
	...
	for(int bit=mxbit;bit>=0;bit--){
		if(is_ancestor(par[u][bit],v))continue;
		u=par[u][bit];
	}
	return par[u][0];
}

pair <mint,mint> calc(int u,int v){
	...
	for(int bit=mxbit;bit>=0;bit--){
		if(((k>>bit)&1)){
			ans = {ans.first + ans.second*val[u][bit].first, ans.second*val[u][bit].second};
			u=par[u][bit];
		}
	}
	return ans;
...
void solve()
{
	...
	for(int i=2;i<=n;i++){
		...
		adj[p].push_back(i);
		par[i][0] = p;
		val[i][0] = {b/deno,(a-b)/deno};
	}

	for(int bit=1;bit<=mxbit;bit++){
		for(int i=2;i<=n;i++){
			par[i][bit]=par[par[i][bit-1]][bit-1];
			val[i][bit]={	val[i][bit-1].first +
							val[i][bit-1].second*val[par[i][bit-1]][bit-1].first,
							val[i][bit-1].second*val[par[i][bit-1]][bit-1].second
						};
		}
	}
	...
	while (q--){
		...
		cin>>u>>v;
		int l = lca(u,v);
		pair <mint,mint> p1 = calc(u,l); 
		pair <mint,mint> p2 = calc(v,l);
		pair <mint,mint> p3 = calc(l,1);
		mint pl = (p3.first+p3.second)*val[1][0].second + (p3.first)*(1-val[1][0].second);
		mint ans = 0;
		ans+= pl*(p1.first+p1.second)*(p2.first+p2.second);
		ans+= (1-pl)*p1.first*p2.first;
		cout<<ans.val()<<" ";
	}cout<<"\n";
...
//	parsed : 15-November-2021 11:21:33 IST