/*
	...
	name : Primes and Queries.cpp
	srcPath : /home/aryan/Documents/cp/Primes_and_Queries.cpp
	url : https://codingcompetitions.withgoogle.com/kickstart/round/00000000004361e3/000000000082bcf4#analysis
*/
/*
    author : aryan57
    created : 12-July-2021 02:38:30 IST
*/
#include <bits/stdc++.h>
using namespace std;
...
void solve_GOOGLE(int tc)
{
    ...
    */
    ...
    F(i, 1, n) {
        int x;
        cin >> x;
        if(x%p==0)tree[0].set(i,f(x));
        else if(x>p)
        {
            tree[1].set(i,1);
            tree[2].set(i,f(x-x%p));
            tree[3].set(i,f(x+x%p)-1);
        }
    }
    ...
    while (q--) {
        ...
        if(op==1){
            ...
            int ans=0;
            ans+=tree[0].prod(l,r+1)*s;
            ans+=tree[1].prod(l,r+1)*f(s);
            ans+=tree[2].prod(l,r+1);

            if(p==2 && s%2==0)
            {
                ans+=tree[3].prod(l,r+1);
            }
            if(q==0)cout<<ans;
            else cout<<ans<<" ";
        }
...
//	parsed : 12-July-2021 02:38:23 IST