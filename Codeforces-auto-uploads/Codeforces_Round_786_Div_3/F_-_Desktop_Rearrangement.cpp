 ...
 *    created: 02.05.2022 21:36:07 IST
**/
#include <bits/stdc++.h>
using namespace std;
...
void solve()
{
	...
	segtree<int,binary_operation,identity_element> t(col);
	vector<segtree<int,binary_operation,identity_element>> t2;
	...
	for(int c=0;c<m;c++){
		...
		t2.push_back(segtree<int,binary_operation,identity_element>(temp));
	}
	...
	while (q--)
	{
		...
		tot+=add;
		t.set(c,t.get(c)+add);
		t2[c].set(r,t2[c].get(r)+add);

		int columns=tot/n;
		int rows=tot%n;

		int sum=t.prod(0,columns)+t2[columns].prod(0,rows);
		cout<<tot-sum<<"\n";
	}
