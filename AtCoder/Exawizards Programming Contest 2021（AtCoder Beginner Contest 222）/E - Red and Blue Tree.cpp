/*
	...
	name : E - Red and Blue Tree.cpp
	srcPath : /home/aryan/Desktop/cp-workspace/E_Red_and_Blue_Tree.cpp
	url : https://atcoder.jp/contests/abc222/tasks/abc222_e
*/
/**
 *    author:  Aryan Agarwal
 *    created: 16.10.2021 11:35:17 IST
**/
...
vector <int> valEdge(mxn);
vector <int> valNode(mxn);
int n,m,k;
vector <int> tin(mxn),tout(mxn);
int T=0;

const int mxbit = 9;
int parent[mxn][mxbit+1];
...
int lca(int u,int v)
{
	...
	if(is_ancestor(parent[u][0],v))return parent[u][0];
	if(is_ancestor(parent[v][0],u))return parent[v][0];

	for(int bit=9;bit>=0;bit--){
		if(is_ancestor(parent[u][bit],v))continue;
		return lca(parent[u][bit],v);
	}
...
void solve()
{
	...
	int sum=accumulate(valEdge.begin(),valEdge.end(),0);
	if((sum+k)%2 || (sum+k)/2 <0 ){
		cout<<0;
		return;
	}
	...
	for(int i=1;i<=n-1;i++)
	{
		...
		{
			dp[i][s] = dp[i-1][s];
			if(s>=valEdge[i])dp[i][s]+=dp[i-1][s-valEdge[i]];
...
//	parsed : 16-October-2021 11:22:41 IST