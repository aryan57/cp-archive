/*
	group : local
	name : Newton-A.cpp
	srcPath : /home/aryan/Desktop/cp-workspace/Newton-A.cpp
	url : /home/aryan/Desktop/cp-workspace/Newton-A.cpp
*/
/**
 *    author:  Aryan Agarwal
 *    created: 27.08.2021 22:49:00 IST
**/
...
const int mxn_fact=1e6;
mint fact[mxn_fact+1];
...
mint calc(int o,int e,int n){
	...
	pre[0]=1;
	for(int j=1;j<=n;j++){
		pre[j]+=ncr(o,j)+pre[j-1];
	}
	mint z=0;
	for(int i=0;i<=e-1;i++){
		z+=ncr(e-1,i)*(pre[o]-pre[i]);
	}
