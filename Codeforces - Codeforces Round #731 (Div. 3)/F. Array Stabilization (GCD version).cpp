/*
	...
	name : F. Array Stabilization (GCD version).cpp
	srcPath : /home/aryan/Documents/cp/F_Array_Stabilization_GCD_version_.cpp
	url : https://codeforces.com/contest/1547/problem/F
*/
/*
    author : aryan57
    created : 10-July-2021 21:00:01 IST
*/
#include <bits/stdc++.h>
using namespace std;
...
void solve_LOL()
{
    ...
    vector <int> v(n);
    for(int &x : v)cin>>x;
    segtree<int,binary_operation,identity_element> tree(v);
    ...
    while (l<=r)
    {
        ...
        F(i,1,n-1)
        {
            ...
            int extra=right-n;
            right=min(right,n);
            int nn1=tree.prod(i,right);
            int nn2=0;
            if(extra>0)
            {
                nn2=tree.prod(0,extra);
            }

            // if(m==1)
            // {
            //     // dbg(i,nn1,nn2,gcd(nn1,nn2),right,extra);
            // }
