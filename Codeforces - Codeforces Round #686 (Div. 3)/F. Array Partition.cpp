/*
	group : Codeforces - Codeforces Round #686 (Div. 3)
	name : F. Array Partition.cpp
	srcPath : /home/aryan/Documents/cp/F_Array_Partition.cpp
	url : https://codeforces.com/problemset/problem/1454/F
*/
/*
    author : aryan57
    created : 05-June-2021 20:23:38 IST
*/
#include <bits/stdc++.h>
using namespace std;
...
int binary_operation_max(int a, int b)
{
    return max(a, b);
}

int identity_element_max()
{
    return -INF;
}
int binary_operation_min(int a, int b)
{
    return min(a, b);
}

int identity_element_min()
{
    return INF;
}
...
segtree<int, binary_operation_max, identity_element_max> mxt(mxn + 1);
segtree<int, binary_operation_min, identity_element_min> mnt(mxn + 1);

void add(int ind, int pos)
{
    mxt.set(ind, max(mxt.get(ind), pos));
    mnt.set(ind, min(mnt.get(ind), pos));
}
void remove(int ind)
{
    auto it = adj[ind].end();
    ...
    {
        mxt.set(ind, max(mxt.get(ind), *adj[ind].begin()));
        mnt.set(ind, min(mnt.get(ind), adj[ind].back()));
    }
}
...
//	parsed : 05-June-2021 19:58:30 IST