/*
	...
	name : D. Priority Queue.cpp
	srcPath : /home/aryan/Documents/cp/D_Priority_Queue.cpp
	url : https://codeforces.com/contest/1542/problem/D
*/
/*
    author : aryan57
    created : 04-July-2021 11:39:52 IST
*/
    ...
    F(i,1,n)
    {
        ...
        F(j,1,n)
        {
            F(k,0,j)
            {
                ...
                {
                    if(j<=i || k>0)
                    dp[j][max(k-1,0ll)]+=dp[j-1][k];
                }
                else
                {
                    if(v[j].Y<v[i].Y || (v[j].Y==v[i].Y && j<i))
                    {
                        dp[j][k+1]+=dp[j-1][k];
                    }
                    else
