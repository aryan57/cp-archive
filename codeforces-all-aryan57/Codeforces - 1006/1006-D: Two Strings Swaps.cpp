...
signed main()
{
    ...
    if(n%2)
    {
        ...
        a=a.substr(0,n/2)+a.substr(n/2+1,n/2);
        b=b.substr(0,n/2)+b.substr(n/2+1,n/2);
        n--;
    ...
    F(i,1,n/2)
    {
        ...
        // }nl;
        if(sz(s)==1)continue;
        if(sz(s)==4){ans+=2;continue;}
        if(sz(s)==2)
        {
            if(a[i]==a[n+1-i] && b[i]!=b[n+1-i])ans++;
            if(a[i]!=a[n+1-i] && b[i]==b[n+1-i])ans++;
            continue;
        }
        if(sz(s)==3)
        {
            if(a[i]==a[n+1-i] && b[i]!=b[n+1-i])ans+=2;
            else ans++;
            continue;
        }
