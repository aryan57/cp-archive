...
const int maxn = (int)2e5; // max no. of nodes
vector <int> v[maxn+1];    // 2d array with maxn+1 rows , each having 0 columns

int depth[maxn+1]={}; // depth of a node
int subtre[maxn+1]={}; // size of subtree of a node

int cnt=1;
vi ans;
vi ind;

// f is the parent element of u
int dfs(int u,int f)
{
    depth[u]=depth[f]+1;
    subtre[u]=1;
    ...
    {
		if (v[u][i]!=f)subtre[u]+=dfs(v[u][i],u); // we could also use if(!visited[v[u][i]]) but it will require an extra array
	}
