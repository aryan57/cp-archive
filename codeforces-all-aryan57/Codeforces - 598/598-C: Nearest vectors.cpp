...
signed main()
{
    ...
    vector < pair<long double,int> > s(n);

    F(i,0,n-1)
    {
        cin>>x>>y;
        s[i]={atan2(y,x),i+1};
        if(s[i].X<0)s[i].X+=2*pie;
    ...
    // F(i,0,n-1)
    // {
    //     cout<<fixed<<setprecision(15)<<s[i].X<<" "<<s[i].Y;nl;
    // }

    // cout<<EPF;nl;


    long double minn=1000.0;
    pii ans;
    long double tt;

    F(i,0,n-1)
    {
        // dbg(s[i%n].X,s[(i+1)%n].X);
        tt=abs(s[i%n].X-s[(i+1)%n].X);
        if(tt>pie)tt=2*pie-tt;
        // cout<<fixed<<setprecision(10)<<minn<<" "<<tt;nl;
        if(minn>=tt)
        {
            minn=tt;
            ans={s[i%n].Y,s[(i+1)%n].Y};
        }
    }
