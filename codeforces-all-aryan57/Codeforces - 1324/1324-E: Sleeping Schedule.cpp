...
signed main()
{
    ...
    F(j,0,n)
    {
        F(i,0,n-1)
        {
            if(((pre[i]-j)%h+h)%h>=l && ((pre[i]-j)%h+h)%h<=r)temp=1;
                else temp=0;
            dp[i+1][j]=max(dp[i+1][j],dp[i][j] + temp);
            if(((pre[i]-j-1)%h+h)%h>=l && ((pre[i]-j-1)%h+h)%h<=r)temp=1;
                else temp=0;
            if(j!=n)dp[i+1][j+1]=max(dp[i+1][j+1],dp[i][j] + temp);
        }
    }

    int ans=0;

    F(j,0,n){ans=max(ans,dp[n][j]);}
