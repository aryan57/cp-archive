...
string s;
vi cnt[26];
void fun(int l,int r,int c,int nas)
{
    if(r==l)
    {
        if(s[l]!=c+'a')nas++;
    ...
    int s1=cnt[c][m]-cnt[c][l-1];
    int s2=cnt[c][r]-cnt[c][m];
        
    fun(m+1,r,c+1,nas+(m-l+1)-s1);
    fun(l,m,c+1,nas+(r-m-1+1)-s2);
...
void solve()
{
    int n;
    cin>>n;
    F(i,0,25){cnt[i].resize(n+1);cnt[i][0]=0;}
    int k=(int)log2(n);
