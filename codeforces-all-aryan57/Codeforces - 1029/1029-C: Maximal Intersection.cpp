...
signed main()
{
    ...
    F(i,0,n-1)
    {
        ...
        if(ok)
        {
            cur.X=max(cur.X,v[i].X);
            cur.Y=min(cur.Y,v[i].Y);
        }
    ...
    F(i,0,n-1)
    {
        // p.erase(p.find(-v[i].X));
        // q.erase(q.find(v[i].Y));
        int x,y;
        int ok1,ok2;
        ok1=ok2=0;
        
        
        int j = lower_bound(p.begin(),p.end(),-cur.X)-p.begin();
        ...
        int k = lower_bound(q.begin(),q.end(),cur.Y)-q.begin();
