...
/*
    ...
    created : 03-August-2020 18:57:23
    // tutorial used after 0 attempt
*/
...
void solve()
{
    ...
    int ans=pre[min(m-1,n-1)];

    F(i,0,m-1)
    {
        ans=max(ans,v[i].X+(n-1)*v[i].Y);

        int j = lower_bound(a.begin(),a.end(),v[i].Y,greater <int> ())-a.begin();
        j--;
        if(j>=0 && j<m)
        {
            int tt=0;
            tt+=pre[min(j,n-1)];
            int rem=n-min(n,j+1);
            if(v[i].X>v[i].Y)ans=max(ans,tt+rem*v[i].Y);
            else ans=max(ans,tt+v[i].X+(rem-1)*v[i].Y);
        }
    }
