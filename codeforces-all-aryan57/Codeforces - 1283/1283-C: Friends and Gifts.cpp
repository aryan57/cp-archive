...
signed main()
{
    ...
    F(i,0,sz(eq)/2-1)
    {
        neq.pb({eq[i].X,eq[sz(eq)-1-i].X});
        neq.pb({eq[sz(eq)-1-i].X,eq[i].X});
    }

    // F(i,0,sz(neq)-1)
    //     {
    //         cout<<neq[i].X<<" "<<neq[i].Y;nl;
    //     }

    if(sz(eq)%2)
    {
        neq.pb({neq[0].X,eq[sz(eq)/2].X});
        neq.pb({eq[sz(eq)/2].X,neq[0].Y});
