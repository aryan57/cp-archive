//https://codeforces.com/contest/69/problem/D

/*
    credits : @rizwanhudda
    sol used....
    author : Aryan Agarwal, IIT KGP
    created : 30-November-2020 16:47:35 IST
*/
...
int fun(int x,int y)
{
    ...
    F(i,0,n-1)
    {
        if((x+v[i].X-200)*(x+v[i].X-200)+(y+v[i].Y-200)*(y+v[i].Y-200)>d*d)continue;
        if(fun(x+v[i].X,y+v[i].Y)==0)
        {
            ok[x][y]=1;
            return 1;
        }
