...
/*
    ...
    created : 24-November-2020 17:55:17 IST
*/
...
const int bfb = 1000000007; /*more than (10)^9*/  /*7 + 1e9*/
const int MOD = 998244353; /*less than (10)^9*/   /*1 + 7*17*(2)^23*/
...
signed main()
{
    ...
    Fo(i,str,n,2)
    {
        ...
        tt*=fact[i+tot-1];
        tt%=MOD;
        tt*=binpow(fact[i-1],MOD-2,MOD);
        tt%=MOD;
        tt*=binpow(fact[tot],MOD-2,MOD);
        tt%=MOD;
        num+=tt;
        num%=MOD;
    }
    ...
    // cout<<num;
    int ans=num;
    int den=binpow(2,n,MOD);
    ans*=binpow(den,MOD-2,MOD);
    ans%=MOD;
    cout<<ans;
