...
void solve()
{
    ...
    long double cur=0.0;
    long double cur2=0.0;
    long double del=0.0;
    long double del2=0.0;
    long double del3=abs(0.5*(h+c)-t);

    if(h+c==2*t){cout<<2;return;}
    ...
    while(k<=xx+1000)
    {
        cur=(1.0)*((h+c)*k+h)/(2*k+1);
        del=abs(cur-t);
        cur2=(1.0)*((h+c)*(k+1)+h)/(2*(k+1)+1);
        del2=abs(cur2-t);

        if(cur2<=(long double)t && cur>=(long double)t)break;
    ...
    if(del<=del2 && del<=del3){cout<<2*k+1;return;}
    if(del2<=del && del2<=del3){cout<<2*(k+1)+1;return;}
    cout<<2;
