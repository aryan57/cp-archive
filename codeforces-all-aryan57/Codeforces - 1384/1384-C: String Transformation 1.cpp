...
void solve()
{
    ...
    vector <unordered_set <int> > cnt1(20);
    vector <unordered_set <int> > cnt2(20);
    ...
    F(i,0,n-1)
    {
        if(a[i]!=b[i])
        {
            cnt1[a[i]-'a'].insert(i);
            cnt2[b[i]-'a'].insert(i);
            if(b[i]>a[i])minn[a[i]-'a']=min(minn[a[i]-'a'],b[i]);
        }
    }
    ...
    for(char c='a';c<='t';c++)
    {
        if(sz(cnt1[c-'a'])>0)
        {
            ...
            ans++;
            vpii p,q,r;
            for(auto it=cnt1[c-'a'].begin();it!=cnt1[c-'a'].end();it++)
            {
                int x=a[*it]-'a';
                int y=b[*it]-'a';
                a[*it]=tt;
            ...
            F(i,0,sz(p)-1)
            {
                cnt1[p[i].X].erase(p[i].Y);
            }
            F(i,0,sz(q)-1)
            {
                cnt2[q[i].X].erase(q[i].Y);
            }
            F(i,0,sz(r)-1)
            {
                cnt1[r[i].X].insert(r[i].Y);
            }
