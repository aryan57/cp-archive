...
signed main()
{
    ...
    if(num/100==0)
    {
        if(num==52 || num==57){cout<<1;return 0;}
        pfn1;
        return 0;
    }
    ...
    F(i,2,n-1)
    {
        F(j,i+1,n)
        {
            if((10*(s[i]-'0')+(s[j]-'0'))%25==0){ans=min(ans,n-j+n-i-1);flag=1;}
            if((10*(s[j]-'0')+(s[i]-'0'))%25==0){ans=min(ans,n-j+n-i);flag=1;}
        }
    }
    ...
    F(j,3,n)
    {
        if((10*(s[1]-'0')+(s[j]-'0'))%25==0)
        {
            temps=s.substr(2,j-2)+s.substr(j+1,n-j)+s[1]+s[j];
            temps="0"+temps;
            F(i,1,n)
            {
                if(temps[i]-'0')
                {
                    ans=min(ans,n-j+n-1-1+i-1);flag=1;
                    break;
                }
            }
        ...
        if((10*(s[j]-'0')+(s[1]-'0'))%25==0)
        {
            temps=s.substr(2,j-2)+s.substr(j+1,n-j)+s[j]+s[1];
            temps="0"+temps;
            F(i,1,n)
            {
                if(temps[i]-'0')
                {
                    ans=min(ans,n-j+n-1+i-1);flag=1;
                    break;
                }
            }
        }
    ...
    if((10*(s[1]-'0')+(s[2]-'0'))%25==0)
    {
        temps=s.substr(3,n-2)+s[1]+s[2];
        temps="0"+temps;
        F(i,1,n)
        {
            if(temps[i]-'0')
            {
                ans=min(ans,n-2+n-1-1+i-1);flag=1;
                break;
            }
        }
    ...
    if((10*(s[2]-'0')+(s[1]-'0'))%25==0)
    {
        temps=s.substr(3,n-2)+s[2]+s[1];
        temps="0"+temps;
        F(i,1,n)
        {
            if(temps[i]-'0')
            {
                ans=min(ans,n-2+n-1+i-1);flag=1;
                break;
            }
        }
