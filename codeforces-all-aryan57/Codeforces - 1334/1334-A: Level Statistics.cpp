...
void solve()
{
    ...
    if(v[0].first<v[0].second){pfn;nl;return;}
    F(i,1,n-1)
    {
        if(v[i].first<v[i].second){pfn;nl;return;}
        if(v[i].first-v[i-1].first<0){pfn;nl;return;}
        if(v[i].second-v[i-1].second<0){pfn;nl;return;}
        if(v[i].first-v[i-1].first-(v[i].second-v[i-1].second)<0){pfn;nl;return;}
    }
    pfy;nl;
}
