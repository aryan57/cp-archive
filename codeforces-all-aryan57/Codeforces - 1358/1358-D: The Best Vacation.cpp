...
signed main()
{
    ...
    int a[2*n+1],sufa[2*n+2],sufsum[2*n+2];
    a[0]=0;
    ...
    F(i,n+1,2*n)
    {
        j = upper_bound(sufa+1,sufa+i+1,k+sufa[i+1],greater <int> ())-sufa-1;
        // watch(j);
        delta=sufa[j]-k-sufa[i+1];
        // dbg(delta,sufa[j],sufa[i+1]);
        ans=max(ans,sufsum[j]-sufsum[i+1]-(delta*(delta+1))/2);
        // watch(ans);
    }
