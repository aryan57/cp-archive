...
/*
    ...
    created : 02-December-2020 17:06:24 IST
*/
...
void solve()
{
    ...
    F(i,1,n)
    {
        if(i-1>0)a[i].left=a[i-1].val;
        if(i+1<=n)a[i].right=a[i+1].val;
    }
    ...
    F(i,1,k)
    {
        int j = posa[b[i]];
        int z=0;
        if(a[j].left!=-1 && !ok[a[j].left])z++;
        if(a[j].right!=-1 && !ok[a[j].right])z++;
        ...
        if(z==1)
        {
            if(a[j].right!=-1  && !ok[a[j].right])
            {
                a[j+1].val=-1;
                a[j].right=a[j+1].right;
                if(j+2<=n)a[j+2].left=a[j+1].left;
            }
            else 
            {
                a[j-1].val=-1;
                if(j-2>=1)a[j-2].right=a[j-1].right;
                a[j].left=a[j-1].left;
            }
