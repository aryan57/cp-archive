...
signed main()
{
    ...
    // F(i,0,sz(a)-1)cout<<a[i]<<" ";nl;
    // F(i,0,sz(b)-1)cout<<b[i]<<" ";nl;
    // F(i,0,sz(ab)-1)cout<<ab[i]<<" ";nl;

    ab.insert(INF);

    // int cura=0,curb=0,curab=0;
    int ans=0;
    while (k--)
    {
        if(*a.begin()+*b.begin()<*ab.begin())
        {
            ans+=*a.begin()+*b.begin();
            a.erase(a.begin());
            b.erase(b.begin());
        ...
        else
        {
            ...
            a.erase(a.find(*ab.begin()));
            b.erase(b.find(*ab.begin()));
            ab.erase(ab.begin());
