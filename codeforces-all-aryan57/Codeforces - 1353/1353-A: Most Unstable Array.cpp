...
#define F(i,a,b) for(ll i=a;a<=b?i<=b:i>=b;a<=b?i+=1:i-=1) // this will manage both increasing & decreasing loops
#define Fo(i,a,b,j) for(ll i=a;a<=b?i<=b:i>=b;a<=b?i+=j:i-=j) // this will manage both increasing & decreasing loops
...
void solve()
{
    ...
    if(n==2){cout<<m;return;}
    if(n>2){cout<<2*m;return;}
}
