...
/*
    ...
    created : 23-November-2020 13:02:17 IST
*/
...
vvi adj(mxn);
int node_a=0,node_b=0,mx=-1,dab=0;

void dfs1(int a,int parent,int b,int cur)
{
    cur++;
    if(a==b)
    {
...
void dfs2(int u,int cur,int par)
{
...
void solve()
{
    ...
    int diameter=mx;
    // dbg(mx);
    if(dab<=da || 2*da>=diameter || 2*da>=db)cout<<"Alice";
    else cout<<"Bob";
