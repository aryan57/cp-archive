...
signed main()
{
    ...
    {
        sum=0;
        int k1=(int)pow(2,k+1);
        int k2=(int)pow(2,k);

        F(i,0,n-1)b[i]=a[i]%k1;
        sort(b,b+n);
        ...
        {
            A=B=C=0;
            j=lower_bound(b+i+1,b+n,k1+k2-b[i])-b;
            if(n>j)A=n-j;
            j=lower_bound(b+i+1,b+n,k1-b[i])-b;
            if(j-i-1>0)B=j-i-1;
            j=lower_bound(b+i+1,b+n,k2-b[i])-b;
            if(j-i-1>0)C=j-i-1;

            sum+=A+B-C;
        }
        if(sum%2)ans+=(int)pow(2,k);
    }
