...
int fun(int lef,int rig)
{
    int l=lower_bound(v1.begin(),v1.end(),lef)-v1.begin();
    int r=lower_bound(v2.begin(),v2.end(),rig, greater <int> ())-v2.begin();
    r=k-1-r;
    ...
    if(szz==0) return min(fun(lef,lef+(rig-lef+1)/2-1)+fun(lef+(rig-lef+1)/2,rig),a);
    else return min(fun(lef,lef+(rig-lef+1)/2-1)+fun(lef+(rig-lef+1)/2,rig),b*(rig-lef+1)*(szz));
}
