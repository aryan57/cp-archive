...
/*
    ...
    created : 23-November-2020 17:21:43 IST
*/
...
void dfs2(int u,int par,vvi &adj)
{
    val[u]=n-val[u];
...
void solve()
{
    ...
    F(i,0,n-1)
    {
        if(val[i]==mn)vec.pb(i);
    }

    if(sz(vec)==1)
    {
        cout<<vec[0]+1<<" "<<adj[vec[0]][0]+1;nl;
        cout<<vec[0]+1<<" "<<adj[vec[0]][0]+1;
    }
    else
    {
        int t=adj[vec[0]][0];
        if(vec[1]==adj[vec[0]][0])t=adj[vec[0]][1];
        cout<<vec[0]+1<<" "<<t+1;nl;
        cout<<vec[1]+1<<" "<<t+1;nl;
